...
class PaymentsBloc extends Bloc<PaymentsEvent, PaymentsState> {
  ...
  PaymentsBloc(DatabaseOperations databaseOperations)
    : super(
        PaymentsState(status: PaymentsStatus.initial, payments: [], error: ''),
      ) {
  ...
  void _onLoadPayments(
    ...
    emit(
      state.copyWith(status: PaymentsStatus.loading, payments: event.payments),
    );
  }
  ...
  Future<void> _onGetPayments(
    ...
    _itemsSubscription = _databaseOperations.paymentsStream().listen(
      (payments) => add(LoadPaymentsEvent(payments)),
    );
  }
